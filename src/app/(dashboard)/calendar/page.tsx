...
export default function CalendarPage() {
  ...
  const formatHeader = () => {
    if (viewMode === 'day') return currentDate.toLocaleDateString('cs-CZ', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })
    if (viewMode === 'week') {
      const days = getWeekDays()
      return `${days[0].toLocaleDateString('cs-CZ', { day: 'numeric', month: 'short' })} — ${days[6].toLocaleDateString('cs-CZ', { day: 'numeric', month: 'short', year: 'numeric' })}`
    }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Calendar className="w-7 h-7 text-blue-600" /> Kalendář
          </h1>
        </div>
      </div>
      ...
      {viewMode === 'day' && (
        <>
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-3 text-center">
              <p className="text-2xl font-bold text-emerald-600">{dayRevenue.toLocaleString('cs-CZ')} Kč</p>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-3 text-center">
              ...
              <p className="text-xs text-gray-500">Volných slotů</p>
            </div>
          </div>
                  ...
                  <div className="flex-1 border-l border-gray-100 min-h-[3rem]">
                    {booking && isStart ? (
                      <button onClick={() => setShowDetail(booking)} className="w-full text-left p-2 hover:brightness-95 transition-all" style={{ minHeight: `${slotCount * 3}rem` }}>
                        <div className="rounded-lg p-2.5 h-full text-white shadow-sm" style={{ backgroundColor: booking.services?.color || '#3b82f6' }}>
                          <div className="flex items-center justify-between">
                            <span className="font-semibold text-sm">{booking.services?.name || 'Rezervace'}</span>
                            <span className="text-xs opacity-80">{booking.services?.duration} min</span>
                          </div>
                          <p className="text-sm opacity-90 mt-0.5">{booking.clients?.full_name || booking.customer_name || 'Klient'}</p>
                      ...
                      <button onClick={() => { if (!isPast) setSelectedSlot({ date: dateStr, time }) }} disabled={isPast}
                        ...
                        {!isPast && <span className="text-xs text-emerald-500 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1"><Plus className="w-3 h-3" /> Nová rezervace</span>}
                      </button>
                    ) : null}
                  </div>
                </div>
            ...
            {getMonthDays().map((day, i) => {
              ...
              return (
                <button key={i} onClick={() => { setCurrentDate(day); setViewMode('day') }}
                  ...
                  {dayB.length > 0 && (
                    <div className="mt-1">
                      <span className="inline-block px-1.5 py-0.5 bg-blue-100 text-blue-700 text-xs rounded-full font-medium">{dayB.length} rez.</span>
                      {revenue > 0 && <p className="text-xs text-gray-500 mt-0.5">{revenue.toLocaleString('cs-CZ')} Kč</p>}
                    </div>
                  )}
                </button>
              )
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-900">Nová rezervace</h3>
            ...
            <div className="bg-blue-50 rounded-xl p-3 mb-4 text-sm">
              <span className="text-blue-600 font-medium">
                {new Date(selectedSlot.date + 'T12:00:00').toLocaleDateString('cs-CZ', { weekday: 'short', day: 'numeric', month: 'long' })} v {selectedSlot.time}
              </span>
            </div>
            <div className="space-y-3">
              <div>
                ...
                <select value={qbService} onChange={e => setQbService(e.target.value)} className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm">
                  ...
                  {services.map(s => <option key={s.id} value={s.id}>{s.name} ({s.duration} min — {s.price} Kč)</option>)}
                </select>
              </div>
              <div>
            ...
            <button onClick={handleQuickBook} disabled={qbSaving || !qbService || !qbName || !qbPhone}
              ...
              {qbSaving ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : '✅ Vytvořit rezervaci'}
            </button>
          </div>
        </div>
      )}
      ...
      {showDetail && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-gray-900">Detail rezervace</h3>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-gray-900">{showDetail.clients?.full_name || showDetail.customer_name || 'Neznámý'}</p>
                  <p className="text-sm text-gray-500">{showDetail.clients?.phone || showDetail.customer_phone || ''}</p>
                </div>
              </div>
              <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex justify-between"><span className="text-gray-500">Služba</span><span className="font-medium">{showDetail.services?.name}</span></div>
                <div className="flex justify-between"><span className="text-gray-500">Čas</span><span className="font-medium">{new Date(showDetail.start_at).toLocaleTimeString('cs-CZ', { hour: '2-digit', minute: '2-digit' })} — {new Date(showDetail.end_at).toLocaleTimeString('cs-CZ', { hour: '2-digit', minute: '2-digit' })}</span></div>
                {showDetail.staff && <div className="flex justify-between"><span className="text-gray-500">Specialista</span><span className="font-medium">{showDetail.staff.full_name}</span></div>}
                {showDetail.price && <div className="flex justify-between"><span className="text-gray-500">Cena</span><span className="font-medium">{showDetail.price} Kč</span></div>}
                <div className="flex justify-between"><span className="text-gray-500">Stav</span><span className={`font-medium ${showDetail.status === 'confirmed' ? 'text-blue-600' : showDetail.status === 'completed' ? 'text-green-600' : 'text-gray-600'}`}>{showDetail.status === 'confirmed' ? 'Potvrzena' : showDetail.status === 'completed' ? 'Dokončena' : showDetail.status}</span></div>
              </div>
            </div>
            <button onClick={() => setShowDetail(null)} className="w-full mt-4 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200">Zavřít</button>
          </div>
        </div>
      )}
    </div>
