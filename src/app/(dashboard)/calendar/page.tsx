...
export default function CalendarPage() {
  ...
  const getBookingsForDate = (date: string) => filteredBookings.filter(b => b.start_at.split('T')[0] === date)
  ...
  const handleQuickBook = async () => {
    ...
    try {
      const res = await fetch('/api/bookings', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          service_id: qbService,
          staff_id: qbStaff || (!isTeam && staffList.length === 1 ? staffList[0].id : null),
          start_at: startDate.toISOString(),
          end_at: endDate.toISOString(),
          customer_name: qbName,
          customer_phone: formatPhone(qbPhone),
          customer_email: qbEmail || undefined,
  ...
  const getStatsForRange = (start: string, end: string) => {
    const rb = filteredBookings.filter(b => {
      const d = b.start_at.split('T')[0]
      ...
      .map(staff => {
        const range =
          viewMode === 'day'
            ? { s: dateStr, e: dateStr }
            : viewMode === 'week'
            ? { s: toDateStr(getWeekDays()[0]), e: toDateStr(getWeekDays()[6]) }
    ...
    <div>
      {backfillMode && (
        <div className="mb-4 p-3 bg-amber-50 border-2 border-amber-300 rounded-xl flex items-center justify-between">
          ...
          <button onClick={() => setBackfillMode(false)} className="text-amber-600 hover:text-amber-800">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
          ...
          <button
            onClick={() =>
              setSelectedSlot({
                ...
                time: `${String(Math.min(Math.max(new Date().getHours(), workStart), workEnd - 1)).padStart(2, '0')}:00`,
              })
            }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={goToday}
            className="px-3 py-1.5 rounded-lg text-sm font-medium border text-white hover:brightness-110"
        ...
        <div className="flex items-center gap-2">
          ...
          {isTeam && (
            <div className="flex items-center gap-1">
              ...
              <select value={filterStaff} onChange={e => setFilterStaff(e.target.value)} className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm bg-white">
            ...
            return (
              <div key={time} className={`flex ${isHour ? 'border-t-2 border-gray-400' : isHalf ? 'border-t border-gray-300' : 'border-t border-gray-200/60'} ${isPast && !booking ? 'bg-gray-100/70' : ''} ${isNow ? 'bg-blue-50/40' : ''}`}>
                ...
                <div className="flex-1 min-h-[2rem]">
                  {booking && isStart ? (
                    <button onClick={() => setShowDetail(booking)} className={`w-full text-left p-1.5 hover:brightness-95 transition-all ${isPast ? 'opacity-75' : ''}`} style={{ minHeight: `${slotCount * 2}rem` }}>
                      <div
                        className={`rounded-lg p-2 h-full text-white shadow-sm ${booking.status === 'completed' ? 'opacity-60 ring-2 ring-green-400' : ''} ${hasConflict(booking) ? 'ring-2 ring-red-500' : ''}`}
          ...
          <table className="w-full" style={{ minWidth: Math.max(600, staffList.length * 180 + 80) + 'px' }}>
            <thead className="sticky top-0 z-20 bg-white shadow-sm">
              <tr className="border-b-2 border-gray-400">
                <th className="w-16 sm:w-20 p-2 text-xs text-gray-400 border-r-2 border-gray-500 bg-gray-50 sticky left-0 z-20" />
                {staffList.map(staff => {
                  ...
                  return (
                    <th key={staff.id} className="p-2 text-center border-r border-gray-400 last:border-r-0 bg-gray-50">
                      <button onClick={() => setFilterStaff(isFiltered ? 'all' : staff.id)} className={`text-sm font-bold transition-all ${isFiltered ? 'text-blue-600 underline' : 'text-gray-800 hover:text-blue-600'}`}>
                        {staff.full_name}
                      </button>
                    ...
                    {staffList.map(staff => {
                      ...
                      return (
                        <td key={staff.id} className={`p-0.5 border-r border-gray-400 last:border-r-0 ${isPast ? 'bg-gray-50/70' : ''}`}>
                          {booking && isStart ? (
                            <button onClick={() => setShowDetail(booking)} className={`w-full text-left p-1 hover:brightness-95 transition-all ${isPast ? 'opacity-75' : ''}`} style={{ minHeight: `${slotCount * 2}rem` }}>
                              <div
                                className={`rounded-md p-1.5 h-full text-white text-xs shadow-sm ${booking.status === 'completed' ? 'opacity-60 ring-2 ring-green-400' : ''} ${hasConflict(booking) ? 'ring-2 ring-red-500' : ''}`}
                                style={{ backgroundColor: booking.services?.color || '#3b82f6' }}
                                data-status={booking.status}
                              >
        ...
        <div ref={calendarRef} className="bg-white rounded-2xl border-2 border-gray-300 overflow-auto shadow-sm" style={{ maxHeight: 'calc(100vh - 300px)' }}>
          <table className="w-full min-w-[700px]">
            <thead className="sticky top-0 z-10 bg-white shadow-sm">
              <tr className="border-b-2 border-gray-400">
                <th className="w-20 p-2 text-xs text-gray-400 border-r-2 border-gray-400 bg-gray-50" />
                          ...
                          <td key={i} className={`p-0.5 align-top border-r border-gray-400 last:border-r-0 ${isT ? 'bg-blue-50/30' : ''} ${isWknd ? 'bg-sky-50/40' : ''} ${isPast && !isWknd && !isT ? 'bg-gray-50/60' : ''}`} style={{ minHeight: '2rem' }}>
                            {slotBookings.length > 0 ? (
                              <div className="space-y-0.5">
                                {slotBookings
                                  .filter(b => isBookingStart(ds, time, b))
                                  .map(b => (
                                    <button
                                      key={b.id}
                              ...
                              <button onClick={() => handleSlotClick(ds, time)} className={`w-full h-full min-h-[2rem] rounded transition-all group flex items-center justify-center ${isPast && !backfillMode ? 'hover:bg-gray-100' : 'hover:bg-emerald-50'}`}>
                                {!isPast && isHour && <Plus className="w-3 h-3 text-emerald-400 opacity-0 group-hover:opacity-100" />}
                              </button>
                            )}
                          </td>
      ...
      {viewMode === 'month' && (
        <div className="bg-white rounded-2xl border-2 border-gray-300 overflow-hidden shadow-sm">
          <div className="grid grid-cols-7 border-b-2 border-gray-400">
            {dayNames.map((d, i) => (
              <div key={d} className={`p-2 text-center text-xs font-bold text-gray-600 border-r border-gray-400 last:border-r-0 ${i >= 5 ? 'bg-sky-50/60' : ''}`}>
                {d}
              </div>
            ))}
          </div>
      ...
      {isTeam && staffSummary.length > 0 && (
        <div className="mt-4 bg-white rounded-2xl border border-gray-200 p-4">
          <h3 className="text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
            <Users className="w-4 h-4 text-violet-600" /> {l.specialist}
          </h3>
              ...
              <div key={s.id} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50 border border-gray-100">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {s.count} {l.rez} | {s.revenue.toLocaleString(locale)} {currency}
                  </p>
                </div>
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setSelectedSlot(null)} className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200">
                <X className="w-4 h-4" />
              </button>
            </div>
            {(() => {
            ...
            <div className="space-y-3">
              <div>
                ...
                >
                  ...
                  {services.map(s => (
                    <option key={s.id} value={s.id}>
                      {s.name} ({s.duration} min - {s.price} {currency})
                    </option>
                  ))}
                </select>
              </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[80vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowSlotBookings(null)} className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {(() => {
              ...
              return canAdd ? (
                <button
                  onClick={() => {
                    const d = showSlotBookings.date
                    const t = showSlotBookings.time || `${String(workStart).padStart(2, '0')}:00`
                    setShowSlotBookings(null)
                    setSelectedSlot({ date: d, time: t })
                  }}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => setShowDetail(null)} className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center hover:bg-gray-200">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="space-y-3">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-xl flex items-center justify-center text-white font-bold" style={{ backgroundColor: showDetail.services?.color || '#3b82f6' }}>
                  {(showDetail.customer_name || showDetail.clients?.full_name || '?')
                    .split(' ')
                    .map((n: string) => n[0])
                    .join('')
                    .toUpperCase()
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{showDetail.services?.name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">{l.date || 'Datum'}</span>
                  <span className="font-medium">{new Date(showDetail.start_at).toLocaleDateString(locale, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">{l.time}</span>
                  <span className="font-medium">
                    {new Date(showDetail.start_at).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' })} - {new Date(showDetail.end_at).toLocaleTimeString(locale, { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                {showDetail.staff && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">{l.specialist}</span>
                    <span className="font-medium">{showDetail.staff.full_name}</span>
                  </div>
                )}
                {showDetail.price && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {showDetail.price} {currency}
                    </span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-gray-500">{l.status}</span>
                  <span className={`font-medium px-2 py-0.5 rounded-full text-xs ${statusColor(showDetail.status)}`}>{statusLabel(showDetail.status)}</span>
                </div>
                {showDetail.is_backfill && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">{l.backfillLabel}</span>
                    <span className="font-medium text-amber-600 px-2 py-0.5 bg-amber-50 rounded-full text-xs">⏱</span>
                  </div>
                )}
                {hasConflict(showDetail) && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">{l.conflict}</span>
                    <span className="font-medium text-red-600 px-2 py-0.5 bg-red-50 rounded-full text-xs">{l.conflict}</span>
                  </div>
                )}
              </div>
              <div className="pt-2">
                {(() => {
                  ...
                  if (isPastBooking) {
                    return <p className="text-xs text-gray-400">{lang === 'en' ? 'Past booking - read only' : 'Minulá rezervace - jen pro čtení'}</p>
                  }
                  ...
                  return (
                    <>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {!isFutureBooking && showDetail.status !== 'completed' && (
                          <button onClick={() => handleStatusChange(showDetail.id, 'completed')} className="px-3 py-2 bg-green-50 text-green-700 rounded-lg text-sm font-medium hover:bg-green-100">
                            {lang === 'en' ? 'Completed' : 'Dokončeno'}
                          </button>
                        )}
                        {showDetail.status !== 'cancelled' && (
                          <button onClick={() => setCancelConfirm({ id: showDetail.id, name: showDetail.customer_name || showDetail.clients?.full_name || '' })} className="px-3 py-2 bg-red-50 text-red-700 rounded-lg text-sm font-medium hover:bg-red-100">
                            {lang === 'en' ? 'Cancel' : 'Zrusit'}
                          </button>
                        )}
                        {!isFutureBooking && showDetail.status !== 'no_show' && (
                          <button onClick={() => handleStatusChange(showDetail.id, 'no_show')} className="px-3 py-2 bg-purple-50 text-purple-700 rounded-lg text-sm font-medium hover:bg-purple-100">
                            {lang === 'en' ? 'No-show' : 'Nedostavil/a se'}
                          </button>
                        )}
                        {showDetail.status !== 'confirmed' && (
                          <button onClick={() => handleStatusChange(showDetail.id, 'confirmed')} className="px-3 py-2 bg-blue-50 text-blue-700 rounded-lg text-sm font-medium hover:bg-blue-100">
            ...
            <button onClick={() => setShowDetail(null)} className="w-full mt-4 py-2.5 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200">
              {l.close}
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[60] p-4" onClick={() => setCancelConfirm(null)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6" onClick={e => e.stopPropagation()}>
            <div className="text-center">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-2">{lang === 'en' ? 'Cancel booking?' : 'Zrušit rezervaci?'}</h3>
              <p className="text-gray-500 text-sm mb-6">
                {cancelConfirm.name && (
                  <>
                    <strong>{cancelConfirm.name}</strong> — 
                  </>
                )}
                {lang === 'en' ? 'The slot will be freed and the client notified.' : 'Slot se uvolní a klient bude informován.'}
              </p>
              <div className="flex gap-3">
                <button onClick={() => setCancelConfirm(null)} className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50">
                  {lang === 'en' ? 'Keep' : 'Ponechat'}
                </button>
                <button
                  onClick={() => {
                    handleStatusChange(cancelConfirm.id, 'cancelled')
