          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
            {SEGMENT_KEYS.map((s, idx) => { const Icon = SEGMENT_ICONS[s.icon]; const isLast = idx === SEGMENT_KEYS.length - 1 && SEGMENT_KEYS.length % 2 !== 0; return (
              <button key={s.key} onClick={() => setOpenSegment(openSegment === s.key ? null : s.key)}
                ...
                <span className={`text-xs mt-1 block font-medium ${openSegment === s.key ? 'text-amber-500' : 'text-amber-400/70 sm:text-amber-400/0 sm:group-hover:text-amber-400/80'} transition-all duration-200`}>▼ {openSegment === s.key ? 'zavřít' : 'služby'}</span>
              </button>
            )})}
          </div>
